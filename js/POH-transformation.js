...
var POHTransformation = function(){
	...
	this.addMoveHouseTriggers = function(){
		$('#moveHouseList li a').overlay({
			onBeforeLoad : function(e){
				...
				return trans.moveHouse(xDirection, positiveDirection);
			},
			closeOnClick : false,
			expose: {
				color: '#333',
	...
	this.moveHouse = function(xDirection, positiveDirection){
		//console.log('moving');
		var yChange = xDirection ? positiveDirection ? -1 : 1 : 0;
		var xChange = xDirection ? 0 : positiveDirection ? -1 : 1;
		
		if(trans.hasRoomThatWouldBeDeletedByMove(xChange, yChange)){
			...
			if(!$house.noConfSettings.demolishOnMove.ignoreWarnings){
				...
				poh.$savedVariables.xChange = xChange;
				poh.$savedVariables.yChange = yChange;
				//show the confirmation dialogue
				return true;
			} //if remembered the 'yes' setting, move the house
			else if ($house.noConfSettings.demolishOnMove.value){
				trans.moveHouseConfirmed(xChange, yChange);
			}
			return false;
		} else {
			trans.moveHouseConfirmed(xChange, yChange);
	...
	this.moveHouseConfirmed = function(xChange, yChange){
		//console.log('really moving');
		$house.transforming = true;
		var tempHouse = poh.init.createEmptyHouseArray();
			...
			for(var j = 0; j < 9; j++){
				for(var k = 0; k < 9; k++){
					if((j + xChange) >= 0 && (k + yChange) >= 0 && (j + xChange) < 9 && (k + yChange) < 9){
						tempHouse[i][j][k] = poh.houseArray[i][j + xChange][k + yChange];
					}
				}
			}
		}
		...
		poh.sc.loadHouseFromSharingCode( poh.sc.calculateSharingCode( tempHouse ) );
		$house.transforming = false;
		//console.log('moved');
	};
	...
	this.hasRoomThatWouldBeDeletedByMove = function(x, y){
		var obstruction = false; //innocent until proven guilty
		if(x === 1){
			obstruction = trans.hasRoomAlongEdge(0, false);
		} else if(x === -1){
			obstruction = trans.hasRoomAlongEdge(8, false);
		}
		if(y === 1){
			//only bother checking if there's been nothing found in the specified x-direction
			if(!obstruction){
				obstruction = trans.hasRoomAlongEdge(0, true);
			}
		} else if(y === -1){
			if(!obstruction){
				obstruction = trans.hasRoomAlongEdge(8, true);
			}
		}
	...
	this.hasRoomAlongEdge = function(sideNumber, x){
		//loop through the floors
		for(var i = 0; i < 3; i++){
			//and one row / column on each floor
			for(var j = 0; j < 9; j++){
				if(x){
					if(poh.houseArray[i][j][sideNumber].labelText !== ' '){
						return true;
					}
				} else {
					if(poh.houseArray[i][sideNumber][j].labelText !== ' '){
						return true;
					}
				}
			}
	...
	*/
	...
	this.transformHouseByMatrix = function(a, b, c, d){
		...
		for(var i = 0; i < 3; i++){
			for(var j = 0; j < 9; j++){
				for(var k = 0; k < 9; k++){
					transCoords = trans.transformRoomByMatrix(j, k, a, b, c, d);
					tempHouse[i][transCoords[0]][transCoords[1]] = poh.houseArray[i][j][k];
					if(poh.houseArray[i][j][k].labelText !== ' '){
						tempHouse[i][transCoords[0]][transCoords[1]].doorLayout = 
							trans.transformDoorOrientation(poh.houseArray[i][j][k].doorLayout, a, b, c, d);
					}
				}
			}
		}
	...
	this.transformDoorOrientation = function(doorLayout, a, b, c, d){
	...
	this.transformRoomByMatrix = function(j, k, a, b, c, d){
		//zero the coordinates of the center of the house
		j -= 4;
		k -= 4;

		var transCoords = trans.transformPointByMatrix(j, k, a, b, c, d);
	...
	this.transformPointByMatrix = function(j, k, a, b, c, d){
	...
	this.rotateRoom = function(clockwise, i, j, k){
		...
		poh.changeRoomOverviewImage($house.csr[0], $house.csr[1], $house.csr[2]);
